...
static GLuint
link_program(GLuint *shaders, int count, bool del)
{
    if(count <= 0)
        return 0;
...
render(struct graphics_context *context)
{
    ...
    glDrawArrays(GL_TRIANGLE_STRIP, 0, context->vert_count);
    glBindVertexArray(0);
    glUseProgram(0);
...
main(int argc, char **argv)
{
    ...
    context.program = link_program((GLuint[2]){vert, frag},2, true);
